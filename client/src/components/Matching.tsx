...
export default function Matching({ room, playerId }: Props) {
  ...
  if (!isRater) {
    ...
    return (
      <div className="w-full max-w-md flex flex-col gap-6 text-center mt-10">
        ...
        <p className="text-slate-400">Gleich wird aufgedeckt, wer was geschrieben hat.</p>
  ...
  function handleClear(answerId: string) {
    setMappings((prev) => { const next = { ...prev }; delete next[answerId]; return next; });
  }
  ...
  return (
    <div className="w-full max-w-md flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        <h3 className="text-xs text-slate-500 uppercase tracking-wide">Antworten — wähle eine aus</h3>
        {round.answers.map((a) => {
          const assignedPlayerId = mappings[a.id];
          const assignedPlayer = assignedPlayerId ? room.players.find((p) => p.id === assignedPlayerId) : null;
          const isSelected = selectedAnswerId === a.id;
          return (
            <button key={a.id} onClick={() => handleAnswerTap(a.id)}
              className={`text-left rounded-lg px-4 py-3 border-2 transition-colors ${
                isSelected ? 'bg-indigo-500/30 border-indigo-400'
                  : assignedPlayer ? 'bg-slate-800 border-emerald-600/50'
                  : 'bg-slate-800 border-slate-700'
              }`}>
              <div className="flex items-start gap-3">
                <div className="flex-1 text-sm">{a.text}</div>
                {assignedPlayer ? (
                  <div className="flex items-center gap-1 text-xs bg-emerald-600/20 text-emerald-200 px-2 py-1 rounded"
                    onClick={(e) => { e.stopPropagation(); handleClear(a.id); }}>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {answerers.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => handlePlayerTap(p.id)} disabled={disabled}
                className={`flex items-center gap-2 rounded-lg px-3 py-3 ${
                  isAssigned ? 'bg-slate-800/30 opacity-40'
                    : selectedAnswerId ? 'bg-slate-700 hover:bg-slate-600'
                    : 'bg-slate-800/50'
                }`}>
