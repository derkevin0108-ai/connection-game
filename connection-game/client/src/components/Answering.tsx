...
export default function Answering({ room, playerId }: Props) {
  ...
  const submittedIds = new Set(
    round.answers.filter((a) => a.playerId).map((a) => a.playerId)
  );

  const depthMeta = DEPTH_LABELS[round.questionDepth];
    ...
    <div className="w-full max-w-md flex flex-col gap-6">
      ...
      <div className="bg-slate-800 rounded-xl p-5 text-center">
        <div className="flex items-center justify-center gap-2 text-xs text-slate-400 mb-3">
          <span>{depthMeta.emoji}</span>
          <span>{CATEGORY_LABELS[round.questionCategory]}</span>
          <span>·</span>
      ...
      <div className="flex items-center gap-2 justify-center text-slate-400 text-sm">
        ...
        <span>
          <strong className="text-slate-200">{rater?.name}</strong> rät diese Runde
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 mt-2">
        ...
        {room.players
          .filter((p) => p.id !== round.raterId)
          .map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-3 bg-slate-800/50 rounded-lg px-4 py-2"
            >
