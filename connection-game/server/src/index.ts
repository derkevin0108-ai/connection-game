...
io.on('connection', (socket) => {
  ...
  socket.on('room:join', ({ playerId, name, emoji, code }, cb) => {
    ...
    cb({
      ok: true,
      room: { ...result.room, game: filterGameForViewer(result.room.game, playerId) },
    });
  ...
  socket.on('game:start', ({ code, playerId }) => {
    ...
    if (!result.ok) {
      console.log(`[room ${code}] start failed: ${result.error}`);
      return;
    }
    console.log(`[room ${code}] Spiel gestartet, Rater: ${room.game.round?.raterId}`);
    broadcastRoom(room);
  });
